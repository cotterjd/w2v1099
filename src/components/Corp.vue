<template>
  <div>
    <div>
     <h3>Corp</h3>
     <label>Revenue 
       <input v-model="revenue" type="text" />
     </label><br>
     <label>Salary 
    ...
    <div> 
      ...
      <span>Corp Taxes: {{corpTaxes}}</span><br>
...
export default {
  ...
  computed: {
    ...
    combinedIncome () {
      return Number(this.revenue) + this.spouseIncome
    },
    //marginalTaxRate () {
    ...
    incomeTaxesOwed () {
      const taxOwed = this.incomeTaxes + this.corpTaxes - this.credits 
